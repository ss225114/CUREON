...
export const AppointmentsProvider = ({ children }) => {
  ...
  const fetchDoctorAppointments = async () => {
    try {
      ...
      const [pendingRes, acceptedRes, allRes] = await Promise.all([
    ...
    (date) => {
      try {
        ...
        return appointments.filter((app) => {
          ...
          return isSameDay(appDate, new Date(date));
        });
      } catch (error) {
        console.error(error);
        return [];
  ...
  const upcomingAppointments = useMemo(() => {
    ...
    return appointments.filter((app) => {
      ...
      return (
        appDate >= today && appDate <= nextTwoDays && app.status === "accepted"
      );
    });
  }, [appointments]);
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      confirmed: todaysApps.filter((app) => app.status === "accepted").length,
